...
def train(log_dir,n_epochs,network_dict,index2token,mode,**kwargs):
    ...
    if mode == "new_model":
        ...
        word_state_out_p = train_helper.permute_encoder_output(encoder_out=word_state_out, perm_mat=perm_mat_pl, batch_size=batch_size, max_word_len=max_lat_word_len)
        mean_state_out_p = train_helper.permute_encoder_output(encoder_out=mean_state_out, perm_mat=perm_mat_pl, batch_size=batch_size, max_word_len=max_lat_word_len)
        logsig_state_out_p = train_helper.permute_encoder_output(encoder_out=logsig_state_out, perm_mat=perm_mat_pl, batch_size=batch_size, max_word_len=max_lat_word_len)
        ...
        train_logits, global_latent_o,global_logsig_o,global_mu_o = decoder.run_decoder(word_sequence_length=sent_word_len_list_pl,train=True,reuse=None,units_lstm_decoder=decoder_dim,lat_words=word_state_out_p,units_dense_global=decoder_dim,char_sequence_length=tf.cast(sent_char_len_list_pl,dtype=tf.int32))
        ...
        word_state_out_p_val = train_helper.permute_encoder_output(encoder_out=word_state_out_val, perm_mat=perm_mat_pl_val, batch_size=batch_size_val, max_word_len=max_lat_word_len)
        mean_state_out_p_val = train_helper.permute_encoder_output(encoder_out=mean_state_out_val, perm_mat=perm_mat_pl_val, batch_size=batch_size_val, max_word_len=max_lat_word_len)
        logsig_state_out_p_val = train_helper.permute_encoder_output(encoder_out=logsig_state_out_val, perm_mat=perm_mat_pl_val, batch_size=batch_size_val, max_word_len=max_lat_word_len)
        ...
        for count,batch in enumerate(inds):
            anneal_c_o,train_predictions_o_np, train_cost_o_np, _, global_step_o_np,summary_inf_train_o=sess.run(
                [anneal_value,train_logits,train_cost,train_step,global_step,tf_summary_train],feed_dict={
                    mask_kl_pl:kl_mask[batch],onehot_words_pl:onehot_words[batch],word_pos_pl:word_pos[batch],perm_mat_pl:perm_mat[batch],
                    sent_word_len_list_pl:lat_sent_len_list[batch],sent_char_len_list_pl:sentence_lens_nchars[batch]})
        ...
        #logger.debug('anneal const {}'.format(anneal_c))
            ...
            if global_step_o_np %100==0:
                ...
                val_predictions_o_np, val_cost_o_np,summary_inf_test_o = sess.run(
                    [test_logits, test_cost,tf_summary_test], feed_dict={mask_kl_pl_val:kl_mask_val[rind],onehot_words_pl_val: onehot_words_val[rind], word_pos_pl_val: word_pos_val[rind],
