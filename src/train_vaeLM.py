...
def train(log_dir,n_epochs,network_dict,index2token,mode,**kwargs):
    ...
    perm_mat,max_lat_word_len,lat_sent_len_list = train_helper.prep_perm_matrix(batch_size=batch_size,word_pos_matrix=word_pos,max_char_len=max_char_len)
    ...
    #placeholders
    mask_kl_pl_1 = tf.placeholder(name='mask_kl_pl', dtype=tf.float32, shape=[batch_size, max_lat_word_len])
    sent_word_len_list_pl_1 = tf.placeholder(name='sent_word_len_list_pl', dtype=tf.int32, shape=[batch_size])
    perm_mat_pl_1 = tf.placeholder(name='perm_mat_pl', dtype=tf.int32, shape=[batch_size, max_lat_word_len])
    onehot_words_pl_1 = tf.placeholder(name='onehot_words_pl', dtype=tf.float32,
                                     shape=[batch_size, max_char_len, vocabulary_size])
    word_pos_pl_1 = tf.placeholder(name='word_pos_pl', dtype=tf.float32, shape=[batch_size, max_char_len])
    sent_char_len_list_pl_1 = tf.placeholder(name='sent_char_len_list_pl', dtype=tf.float32, shape=[batch_size])

    mask_kl_pl_val_1 = tf.placeholder(name='mask_kl_pl_val', dtype=tf.float32, shape=[batch_size, max_lat_word_len])
    sent_word_len_list_pl_val_1 = tf.placeholder(name='sent_word_len_list_pl_val', dtype=tf.int32, shape=[batch_size])
    perm_mat_pl_val_1 = tf.placeholder(name='perm_mat_pl_val', dtype=tf.int32, shape=[batch_size, max_lat_word_len])
    onehot_words_pl_val_1 = tf.placeholder(name='onehot_words_pl_val', dtype=tf.float32,
                                         shape=[batch_size, max_char_len, vocabulary_size])
    word_pos_pl_val_1 = tf.placeholder(name='word_pos_pl_val', dtype=tf.float32, shape=[batch_size, max_char_len])
    sent_char_len_list_pl_val_1 = tf.placeholder(name='sent_char_len_list_pl_val', dtype=tf.float32, shape=[batch_size])

    #gaussian samples seed
    samples = np.random.normal(size=[batch_size, decoder_dim])
    ...
    if mode == "new_model":
        ...
        word_state_out, mean_state_out, logsig_state_out = encoder_k.run_encoder(sentence_lens=sent_char_len_list_pl_1,train=True,inputs=onehot_words_pl_1, word_pos=word_pos_pl_1,reuse=None)
        ...
        word_state_out_p = train_helper.permute_encoder_output(encoder_out=word_state_out, perm_mat=perm_mat_pl_1, batch_size=batch_size, max_word_len=max_lat_word_len)
        mean_state_out_p = train_helper.permute_encoder_output(encoder_out=mean_state_out, perm_mat=perm_mat_pl_1, batch_size=batch_size, max_word_len=max_lat_word_len)
        logsig_state_out_p = train_helper.permute_encoder_output(encoder_out=logsig_state_out, perm_mat=perm_mat_pl_1, batch_size=batch_size, max_word_len=max_lat_word_len)
        ...
        train_logits, global_latent_o,global_logsig_o,global_mu_o = decoder.run_decoder(word_sequence_length=sent_word_len_list_pl_1,train=True,reuse=None,units_lstm_decoder=decoder_dim,lat_words=word_state_out_p,units_dense_global=decoder_dim,char_sequence_length=tf.cast(sent_char_len_list_pl_1,dtype=tf.int32))
        ...
        #calculate cost
        train_cost,reconstruction,kl_p3,kl_p1,kl_global,kl_p2,anneal_value,_ = decoder.calc_cost(eow_mask=None,mask_kl=mask_kl_pl_1,kl=True,sentence_word_lens=sent_word_len_list_pl_1,shift=shift,total_steps=total_steps,global_step=global_step,global_latent_sample=global_latent_o,global_logsig=global_logsig_o,global_mu=global_mu_o,predictions=train_logits,true_input=onehot_words_pl_1,posterior_logsig=logsig_state_out_p,posterior_mu=mean_state_out_p,post_samples=word_state_out_p,reuse=None)
        ...
        #testing graph
        word_state_out_val, mean_state_out_val, logsig_state_out_val = encoder_k.run_encoder(sentence_lens=sent_char_len_list_pl_val_1,train=False,inputs=onehot_words_pl_val_1, word_pos=word_pos_pl_val_1,reuse=True)
        ...
        word_state_out_p_val = train_helper.permute_encoder_output(encoder_out=word_state_out_val, perm_mat=perm_mat_pl_val_1, batch_size=batch_size_val, max_word_len=max_lat_word_len)
        mean_state_out_p_val = train_helper.permute_encoder_output(encoder_out=mean_state_out_val, perm_mat=perm_mat_pl_val_1, batch_size=batch_size_val, max_word_len=max_lat_word_len)
        logsig_state_out_p_val = train_helper.permute_encoder_output(encoder_out=logsig_state_out_val, perm_mat=perm_mat_pl_val_1, batch_size=batch_size_val, max_word_len=max_lat_word_len)

        #decode test
        test_logits, global_latent_o_val,global_logsig_o_val,global_mu_o_val = decoder.run_decoder(word_sequence_length=sent_word_len_list_pl_val_1,train=False,reuse=True,units_lstm_decoder=decoder_dim,lat_words=mean_state_out_p_val,units_dense_global=decoder.global_lat_dim,char_sequence_length=tf.cast(sent_char_len_list_pl_val_1,dtype=tf.int32))
        test_logits = tf.identity(test_logits, name="test_logits")

        #test cost
        test_cost = decoder.test_calc_cost(mask_kl=mask_kl_pl_val_1,sentence_word_lens=sent_word_len_list_pl_val_1,posterior_logsig=logsig_state_out_p_val,post_samples=word_state_out_p_val,global_mu=global_mu_o_val,global_logsig=global_logsig_o_val,global_latent_sample=global_latent_o_val,posterior_mu=mean_state_out_p_val,true_input=onehot_words_pl_val_1,predictions=test_logits)
        ...
        saver.restore(sess, tf.train.latest_checkpoint('/Users/kevindsouza/Documents/UBC/Research/ML/VaeLM/model/saved'))
        ...
        # train placeholders
        mask_kl_pl_1 = graph.get_tensor_by_name("mask_kl_pl:0")
        onehot_words_pl_1 = graph.get_tensor_by_name("onehot_words_pl:0")
        word_pos_pl_1 = graph.get_tensor_by_name("word_pos_pl:0")
        perm_mat_pl_1 = graph.get_tensor_by_name("perm_mat_pl:0")
        sent_word_len_list_pl_1 = graph.get_tensor_by_name("sent_word_len_list_pl:0")
        sent_char_len_list_pl_1 = graph.get_tensor_by_name("sent_char_len_list_pl:0")
        ...
        # test placeholders
        mask_kl_pl_val_1 = graph.get_tensor_by_name("mask_kl_pl_val:0")
        onehot_words_pl_val_1 = graph.get_tensor_by_name("onehot_words_pl_val:0")
        word_pos_pl_val_1 = graph.get_tensor_by_name("word_pos_pl_val:0")
        perm_mat_pl_val_1 = graph.get_tensor_by_name("perm_mat_pl_val:0")
        sent_word_len_list_pl_val_1 = graph.get_tensor_by_name("sent_word_len_list_pl_val:0")
        sent_char_len_list_pl_val_1 = graph.get_tensor_by_name("sent_char_len_list_pl_val:0")
        ...
        for count,batch in enumerate(inds):
            anneal_c_o,train_predictions_o_np, train_cost_o_np, _, global_step_o_np,summary_inf_train_o=sess.run(
                [anneal_value,train_logits,train_cost,train_step,global_step,tf_summary_train],feed_dict={
                    mask_kl_pl_1:kl_mask[batch],onehot_words_pl_1:onehot_words[batch],word_pos_pl_1:word_pos[batch],perm_mat_pl_1:perm_mat[batch],
                    sent_word_len_list_pl_1:lat_sent_len_list[batch],sent_char_len_list_pl_1:sentence_lens_nchars[batch]})
        ...
        #logger.debug('anneal const {}'.format(anneal_c))
            ...
            if global_step_o_np %100==0:
                ...
                val_predictions_o_np, val_cost_o_np,summary_inf_test_o = sess.run(
                    [test_logits, test_cost,tf_summary_test], feed_dict={mask_kl_pl_val_1:kl_mask_val[rind],onehot_words_pl_val_1: onehot_words_val[rind], word_pos_pl_val_1: word_pos_val[rind],
                                         perm_mat_pl_val_1: perm_mat_val[rind], sent_word_len_list_pl_val_1: lat_sent_len_list_val[rind],
                                         sent_char_len_list_pl_val_1: sentence_lens_nchars_val[rind]})
