...
def train(log_dir,n_epochs,network_dict,index2token,**kwargs):
    ...
    out_o = tf.identity(out_o, name="train_logits")
    ...
    out_o_val = tf.identity(out_o_val, name="test_logits")
    #test cost
    ...
    ######
    #tensorboard stuff
    summary_inf_train = tf.summary.merge([decoder.kls_hist,decoder.global_kl_scalar,decoder.rec_scalar,decoder.full_kl_scalar,decoder.sum_all_activ_hist,decoder.sum_global_activ_hist])
    summary_inf_train = tf.identity(summary_inf_train,name="tf_summary_train")
    summary_inf_test = tf.summary.merge([decoder.sum_rec_val,decoder.sum_kl_val])
    summary_inf_test = tf.identity(summary_inf_train, name="tf_summary_test")
    ...
    for epoch in range(1):
        ...
        for count,batch in enumerate(inds):
            anneal_c_o,train_predictions_o_np, train_cost_o_np, _, global_step_o_np,train_rec_cost_o_np,_,_,_,_,summary_inf_train_o=sess.run(
                [anneal,out_o,cost,train_step,global_step,reconstruction,kl_p3,kl_p1,kl_global,kl_p2,summary_inf_train],feed_dict={
