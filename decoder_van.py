...
#        kl_global_lat = 0.5*(tf.reduce_sum(tf.exp(global_logsig),axis=-1)+ tf.reduce_sum((global_mu*global_mu),axis=-1)-tf.cast(tf.shape(global_mu)[-1],dtype=tf.float32)-tf.reduce_sum(global_logsig))
    ...
    def generation_van(self,samples):
            predictions= self.vanilla_decoder(samples, reuse=True)
            return predictions
    ...
    def vanilla_cost_function(self,predictions,true_input,posterior_mu,posterior_logsig,shift,total_steps,global_step,kl=True):
        mask = tf.reduce_sum(true_input, -1)
        #reconstruction = tf.reduce_sum(tf.reduce_sum(-true_input*tf.log(predictions+1e-9),axis=-1),-1)
        reconstruction = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.argmax(true_input,-1),logits=predictions)*mask,-1)
        #have to be very careful of order of the mean/stddev parmeters
        ...
        kl_p1 = 0.5*tf.reduce_sum(1+2*posterior_logsig-2*posterior_mu-tf.square(tf.exp(posterior_logsig)))
        if kl:
            kl_p3 = kl_p1 + kl_global_lat
            anneal_c = tf.cast(tf.minimum(tf.maximum(tf.divide((global_step-shift),total_steps),0),1),dtype=tf.float32)
            kl_p3 = kl_p3*anneal_c
        ...
        cost = tf.reduce_mean(kl_p3+reconstruction)
        self.sum_rec_val = tf.summary.scalar('rec_train',tf.reduce_mean(reconstruction))
        self.sum_kl_val = tf.summary.scalar('kl_train',tf.reduce_mean(kl_p1))
        self.sum_val_cost = tf.summary.scalar('cost_train',cost)
        return cost,reconstruction,kl_p3,kl_p1,anneal_c

    def vanilla_test_cost_function(self,predictions,true_input,posterior_mu,posterior_logsig):
        ...
        kl_p1 = 0.5*tf.reduce_sum(1+2*posterior_logsig-2*posterior_mu-tf.square(tf.exp(posterior_logsig)))
        cost = tf.reduce_mean(kl_p1+reconstruction)
        self.sum_rec_val = tf.summary.scalar('rec_test',tf.reduce_mean(reconstruction))
        self.sum_kl_val = tf.summary.scalar('kl_test',tf.reduce_mean(kl_p1))
        self.sum_val_cost = tf.summary.scalar('cost_test',cost)
        return cost,reconstruction,kl_p1
