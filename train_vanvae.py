...
def train(log_dir,n_epochs,network_dict,index2token,**kwargs):
    ...
    lat_var, mu, logsig = encoder.vanilla_encoder(inputs=onehot_words_pl, seq_length=encoder.sentence_lens_nchars, reuse=None)
    ...
    ####
    cost = decoder.vanilla_cost_function(predictions=predictions, true_input=onehot_words_pl, posterior_mu=mu, posterior_logsig=logsig, shift=shift, total_steps=total_steps,
    ...
    lat_var_val, mu_val, logsig_val = encoder.vanilla_encoder(inputs=onehot_words_pl, seq_length=encoder.sentence_lens_nchars, reuse=True)
    predictions_val = decoder.vanilla_decoder(inputs=mu_val, reuse=True)

    #test cost
    cost = decoder.vanilla_test_cost_function(predictions=predictions_val, true_input=onehot_words_pl_val, posterior_mu=mu_val, posterior_logsig=logsig_val)
    ######
    ...
    for epoch in range(n_epochs):
        ...
        for count,batch in enumerate(inds):
            anneal_c_o,train_predictions_o_np, train_cost_o_np, _, global_step_o_np,summary_inf_train_o=sess.run([anneal,predictions,cost,train_step,global_step,summary_inf_train],feed_dict={onehot_words_pl:onehot_words[batch],sent_char_len_list_pl:sentence_lens_nchars[batch]})
	    #logger.debug('anneal const {}'.format(anneal_c))
            ...
            if global_step_o_np % 1000==0:
                ...
                val_predictions_o_np, val_cost_o_np,summary_inf_test_o = sess.run(
                    [out_o_val, test_cost,summary_inf_test], feed_dict={onehot_words_pl_val: onehot_words_val[rind],sent_char_len_list_pl_val: sentence_lens_nchars_val[rind]})
