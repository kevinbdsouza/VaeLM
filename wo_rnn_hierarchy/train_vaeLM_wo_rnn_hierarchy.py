...
def train(log_dir, n_epochs, network_dict, index2token, **kwargs):
    ...
    ######
    # prior sampling
    # changed to sample from normal everytime
    global_sample = np.random.normal(size=[batch_size, decoder.global_lat_dim])
    latent_words = np.random.normal(size=[batch_size, decoder.max_num_words, decoder.lat_word_dim])
    gen_samples = decoder.generation(samples=global_sample, latent_words=latent_words)
        ...
        for count, batch in enumerate(inds):
            anneal_c_o, train_predictions_o_np, train_cost_o_np, _, global_step_o_np, train_rec_cost_o_np, _, _, _, _, summary_inf_train_o = sess.run(
                [anneal, out_o, cost, train_step, global_step, reconstruction, kl_p3, kl_p1, kl_global, kl_p2,
                 summary_inf_train], feed_dict={onehot_words_pl: onehot_words[batch], word_pos_pl: word_pos[batch],
