...
#	_inputs_ta = _inputs_ta.unstack(tf.transpose(inputs,[1,0,2]))
    ...
    def cost_function(self,predictions,true_input,global_mu,global_logsig,posterior_mu,posterior_logsig,shift,total_steps,global_step,kl=True):
        ...
        prior_mu = tf.zeros([self.batch_size,self.max_num_words,self.lat_word_dim],dtype=tf.float32)
        ...
        kl_p1 = 0.5 * (-tf.reduce_sum(tf.log(tf.square(tf.exp(posterior_logsig))),axis=-1) - 
            ...
            tf.reduce_sum(tf.square(tf.exp(posterior_logsig)),axis=-1) + tf.reduce_sum((posterior_mu * posterior_mu), axis=-1))
    ...
    def test_cost_function(self,predictions,true_input,global_mu,global_logsig,posterior_mu,posterior_logsig):
        mask = tf.reduce_sum(true_input,-1)
        reconstruction = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.argmax(true_input, -1), logits=predictions)*mask, -1)
...
#        kl_global_lat = 0.5*(tf.reduce_sum(tf.exp(global_logsig),axis=-1)+ tf.reduce_sum((global_mu*global_mu),axis=-1)-tf.cast(tf.shape(global_mu)[-1],dtype=tf.float32)-tf.reduce_sum(global_logsig))
    ...
    def calc_cost(self,kl,posterior_logsig,post_samples,global_mu,global_logsig,global_latent_sample,posterior_mu,true_input,sentence_word_lens,predictions,shift, total_steps, global_step,reuse):
        ...
        cost, reconstruction, kl_p3, kl_p1,kl_global,kl_p2,anneal_c= self.cost_function(kl=kl,predictions=predictions,true_input=true_input,global_mu=global_mu,global_logsig=global_logsig,posterior_mu=posterior_mu,posterior_logsig=posterior_logsig,shift=shift, total_steps=total_steps, global_step=global_step)
    ...
    def test_calc_cost(self,posterior_logsig,post_samples,global_mu,global_logsig,global_latent_sample,posterior_mu,true_input,predictions,sentence_word_lens):
        ...
        cost,_,_,_ = self.test_cost_function(predictions=predictions,true_input=true_input,global_mu=global_mu,global_logsig=global_logsig,posterior_mu=posterior_mu,posterior_logsig=posterior_logsig)
        return cost

    def generation(self,samples,latent_words):
