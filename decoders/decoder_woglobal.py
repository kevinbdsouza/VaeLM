...
class Decoder:
        ...
        def loop_fn(time, cell_output, cell_state, loop_state):
            ...
            if cell_output is None:  # time == 0
                ...
                context = self.bahd_attention(
                    ...
                    reuse=None)
                next_input = tf.concat([tf.zeros(shape=[self.batch_size, self.dict_length], dtype=tf.float32),
                                        tf.zeros(shape=[self.batch_size, self.global_lat_dim], dtype=tf.float32)],
                                       axis=-1)
    ...
    def run_decoder(self, units_lstm_decoder, sequence_length, units_dense_global, lat_words, reuse):
        if self.simple_decoder:
            ...
            out = self.decoder_p2(sequence_length=sequence_length, num_hidden_word_units=units_lstm_decoder,
    ...
    def generation(self, samples):
        ...
        with tf.variable_scope('prior', reuse=True):
            _, _, loop_state_ta = tf.nn.raw_rnn(cell, loop_fn)
            loop_state_out = _transpose_batch_time(loop_state_ta.stack())
        predictions = self.decoder_p2(num_hidden_word_units=self.lat_word_dim, inputs=loop_state_out,
