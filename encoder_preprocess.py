...
class Encoder:
    ...
    def run_encoder(self,inputs,word_pos,reuse):
        with tf.variable_scope('preprocess_rnn',reuse=reuse):
            fw = tf.contrib.rnn.LSTMCell(128)
            bw = tf.contrib.rnn.LSTMCell(128)
            inputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw, cell_bw=bw, inputs=inputs, dtype=tf.float32)
        inputs_t = tf.concat(inputs, -1)
        inputs_t.set_shape((self.batch_size, self.max_char_len, 256))
        inputs_t = tf.transpose(inputs_t, perm=[1, 0, 2])
        inputs_t.set_shape((self.max_char_len, self.batch_size, 256))
        ...
        def loop_fn(time, cell_output, cell_state, loop_state):
            ...
            if cell_output is None:  # time = 0
                ...
                #next_input.set_shape((self.batch_size,1,256))
