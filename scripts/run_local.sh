...
def experiment(exp_name):
	from decoder import Decoder
        ...
        onehot_words,word_pos,sentence_lens_nchars,sentence_lens_nwords,vocabulary_size,max_char_len=read(file_name='/share/storage/nVAE/train.h5', train=True)
        onehot_words_val, word_pos_val, sentence_lens_nchars_val, sentence_lens_nwords_val, _, _ =read(file_name='/share/storage/nVAE/test.h5', train=False)
	from cluster_print import log
        max_char_len = 494
        batch_size = 52
        hidden_size = 1024
        decoder_dim = 1024
	...
	log('start')
	train(log_dir='/share/storage/nVAE/t1_1024/',n_epochs=1,network_dict=network_dict,index2token=index2token,**train_dict)
	log("end")
