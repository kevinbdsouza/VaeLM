...
def experiment(exp_name):
	from decoder import Decoder
        ...
        onehot_words,word_pos,sentence_lens_nchars,sentence_lens_nwords,vocabulary_size,max_char_len=read(file_name='/share/storage/zalperstein/nVAE/train.h5', train=True)
        onehot_words_val, word_pos_val, sentence_lens_nchars_val, sentence_lens_nwords_val, _, _ =read(file_name='/share/storage/zalperstein/nVAE/test.h5', train=False)
	...
	train(log_dir='/share/storage/zalperstein/nVAE/0401_nokl_lr1e3_clip_512_detinit_decxl_vae_an2/',n_epochs=500,network_dict=network_dict,index2token=index2token,**train_dict)
...
if __name__ == '__main__':
    ...
    pool.attach_files(['train_vaeLM.py','decoder.py','encoder.py','cluster_print.py','pre.py',h5py,'cluster_print.py'])
