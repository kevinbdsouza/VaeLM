...
def eow_pos(word_pos,sample_pos,data_size,max_char_len):
    word_locations = np.zeros([data_size],dtype=np.float32)
	...
	def run_encoder(self,inputs,word_pos,word_locations,reuse):
		...
		def loop_fn(time, cell_output, cell_state, loop_state):
			...
			if cell_output is None:  # time = 0
				...
				comp_tensor = tf.multiply(tf.ones([self.batch_size],dtype=tf.float32),tf.cast(time-1,dtype=tf.float32))
				sample_mask = tf.cast(tf.equal(word_locations,comp_tensor),dtype=tf.float32)
				other_words_mask = word_pos[:,time-1] - sample_mask
				sample_mask = tf.cast(tf.transpose(tf.reshape(tf.tile(sample_mask,[self.hidden_size]),[self.hidden_size,self.batch_size]),perm=[1,0]),dtype=tf.float32)
				other_words_mask = tf.cast(tf.transpose(tf.reshape(tf.tile(other_words_mask,[self.hidden_size]),[self.hidden_size,self.batch_size]),perm=[1,0]),dtype=tf.float32)
