...
def train(log_dir, n_epochs, network_dict, index2token, **kwargs):
    ...
    for batch,batch_len in enumerate(sentence_lens_nwords):
        ...
        for ex, len in enumerate(batch_len):
            onehot_words[batch, ex, :,27] = onehot_words[batch, ex, :,27]*np.true_divide(len,2)
    ...
    sum_grad_hist = [tf.summary.histogram(name=regex.sub('', str(j)), values=i) for i, j in
                     zip(clipped_grads_t, vars_t)]
    ...
    summary_inf_train = tf.summary.merge(
        [sum_grad_hist, norm_grad, decoder.kls_hist, decoder.global_kl_scalar, decoder.rec_scalar, decoder.cost_scalar,
         decoder.full_kl_scalar, decoder.sum_all_activ_hist, decoder.sum_global_activ_hist])
