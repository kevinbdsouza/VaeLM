...
class Encoder:
    ...
    def vanilla_encoder(self, inputs, seq_length, reuse):
        ...
        with tf.variable_scope('lat_var', reuse=reuse):
            out = tf.layers.dense(inputs=state[-1], units=self.lat_dim * 2, activation=tf.nn.relu)
            mu, logsig = tf.split(tf.layers.dense(inputs=out, units=self.lat_dim * 2, activation=None), 2, axis=-1)
