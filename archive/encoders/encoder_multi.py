...
class Encoder:
    ...
    def run_encoder(self, train, inputs, word_pos, sentence_lens, reuse):
        ...
        with tf.variable_scope('encoder_bi', reuse=reuse):
            ...
            cells_fw = [cell(self.input_size) for _ in range(2)]
            cells_bw = [cell(self.input_size) for _ in range(2)]
            values, _, _ = tf.contrib.rnn.stack_bidirectional_dynamic_rnn(inputs=inputs, dtype=tf.float32, cells_bw=cells_bw,
                                                             cells_fw=cells_fw, sequence_length=sentence_lens)
