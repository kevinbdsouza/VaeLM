...
def permute_encoder_output(encoder_out, perm_mat, batch_size, max_word_len):
    ...
    o = tf.stack(
        [tf.gather(params=i, indices=j, axis=0) for i, j in zip(tf.unstack(o, axis=0), tf.unstack(perm_mat, axis=0))])
    return o
...
def train(log_dir, n_epochs, network_dict, index2token, **kwargs):
    ...
    perm_mat, max_word_len, sent_len_list = prep_perm_matrix(batch_size=batch_size, word_pos_matrix=word_pos,
    ...
    onehot_words_pl = tf.placeholder(name='onehot_words', dtype=tf.float32,
    ...
    arg_dict = {'decoder_p3_units': decoder_units_p3, 'encoder_dim': hidden_size, 'lat_word_dim': hidden_size,
                ...
                'max_num_words': max_word_len, 'decoder_units': decoder_dim, 'num_sentence_characters': max_char_len,
    ...
    word_state_out, mean_state_out, logsig_state_out = encoder_k.run_encoder(inputs=onehot_words_pl,
    ...
    word_state_out_p = permute_encoder_output(encoder_out=word_state_out, perm_mat=perm_mat_pl, batch_size=batch_size,
                                              max_word_len=max_word_len)
    mean_state_out_p = permute_encoder_output(encoder_out=mean_state_out, perm_mat=perm_mat_pl, batch_size=batch_size,
                                              max_word_len=max_word_len)
    logsig_state_out_p = permute_encoder_output(encoder_out=logsig_state_out, perm_mat=perm_mat_pl,
                                                batch_size=batch_size, max_word_len=max_word_len)
    # Initialize decoder
    ...
    out_o, global_latent_o, global_logsig_o, global_mu_o = decoder.run_decoder(reuse=None,
    ...
    word_loc_val = np.reshape(word_loc_val[0:n_valid_use], newshape=[-1, batch_size_val])
    ...
    ####
    cost, reconstruction, kl_p3, kl_p1, kl_global, kl_p2, anneal = decoder.calc_cost(kl=True,
    ...
    onehot_words_pl_val = tf.placeholder(name='onehot_words_val', dtype=tf.float32,
                                         shape=[batch_size, max_char_len, vocabulary_size])
    ...
    word_loc_pl_val = tf.placeholder(name='word_loc_val', dtype=tf.float32, shape=[batch_size])

    # testing graph
    word_state_out_val, mean_state_out_val, logsig_state_out_val = encoder_k.run_encoder(inputs=onehot_words_pl_val,
                                                                                         word_pos=word_pos_pl_val,
                                                                                         reuse=True)
    perm_mat_val, _, sent_len_list_val = prep_perm_matrix(batch_size=batch_size_val, word_pos_matrix=word_pos_val,
                                                          max_char_len=max_char_len, max_word_len=max_word_len)
    ...
    word_state_out_p_val = permute_encoder_output(encoder_out=word_state_out_val, perm_mat=perm_mat_pl_val,
                                                  batch_size=batch_size_val, max_word_len=max_word_len)
    mean_state_out_p_val = permute_encoder_output(encoder_out=mean_state_out_val, perm_mat=perm_mat_pl_val,
                                                  batch_size=batch_size_val, max_word_len=max_word_len)
    logsig_state_out_p_val = permute_encoder_output(encoder_out=logsig_state_out_val, perm_mat=perm_mat_pl_val,
                                                    batch_size=batch_size_val, max_word_len=max_word_len)
    out_o_val, global_latent_o_val, global_logsig_o_val, global_mu_o_val = decoder.run_decoder(reuse=True,
                                                                                               units_lstm_decoder=decoder_dim,
                                                                                               ...
                                                                                               sequence_length=tf.cast(
                                                                                                   ...
                                                                                                   dtype=tf.int32))
    # test cost
    test_cost = decoder.test_calc_cost(sentence_word_lens=sent_word_len_list_pl_val,
                                       ...
                                       true_input=onehot_words_pl_val, predictions=out_o_val)
    ...
    summary_inf_train = tf.summary.merge(
        [decoder.kls_hist, decoder.global_kl_scalar, decoder.rec_scalar, decoder.cost_scalar, decoder.full_kl_scalar,
        ...
        for count, batch in enumerate(inds):
            anneal_c_o, train_predictions_o_np, train_cost_o_np, _, global_step_o_np, train_rec_cost_o_np, _, _, _, _, summary_inf_train_o = sess.run(
                [anneal, out_o, cost, train_step, global_step, reconstruction, kl_p3, kl_p1, kl_global, kl_p2,
                 summary_inf_train],
                feed_dict={onehot_words_pl: onehot_words[batch], word_pos_pl: word_pos[batch],
                           perm_mat_pl: perm_mat[batch], sent_word_len_list_pl: sentence_lens_nwords[batch],
                           sent_char_len_list_pl: sentence_lens_nchars[batch], word_loc_pl: word_loc[batch]})
            ...
            if count % 1000 == 0:
                ...
                val_predictions_o_np, val_cost_o_np = sess.run(
                    [out_o_val, test_cost],
                    feed_dict={onehot_words_pl_val: onehot_words_val[0], word_pos_pl_val: word_pos_val[0],
