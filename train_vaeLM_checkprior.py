...
def train(log_dir, n_epochs, network_dict, index2token, **kwargs):
    ...
    ####
    cost, reconstruction, kl_p3, kl_p1, kl_global, kl_p2, anneal, prior_mu_o = decoder.calc_cost(mask_kl=mask_kl_pl,
                                                                                                 kl=True,
    ...
    pr_out_o, pr_global_latent_o, pr_global_logsig_o, pr_global_mu_o = decoder.run_decoder(reuse=True,
                                                                                           units_lstm_decoder=decoder_dim,
                                                                                           lat_words=prior_mu_o,
                                                                                           units_dense_global=decoder_dim,
                                                                                           sequence_length=tf.cast(
                                                                                               sent_char_len_list_pl,
                                                                                               dtype=tf.int32))
    ...
    word_state_out_p_val = permute_encoder_output(encoder_out=word_state_out_val, perm_mat=perm_mat_pl_val,
                                                  batch_size=batch_size_val, max_word_len=max_lat_word_len)
    ...
    out_o_val, global_latent_o_val, global_logsig_o_val, global_mu_o_val = decoder.run_decoder(reuse=True,
                                                                                               ...
                                                                                               sequence_length=tf.cast(
                                                                                                   ...
                                                                                                   dtype=tf.int32))
    # test cost
    test_cost = decoder.test_calc_cost(mask_kl=mask_kl_pl, sentence_word_lens=sent_word_len_list_pl_val,
    ...
    for epoch in range(n_epochs):
        ...
        for count, batch in enumerate(inds):
            pr_out_predictions, anneal_c_o, train_predictions_o_np, train_cost_o_np, _, global_step_o_np, train_rec_cost_o_np, _, _, _, _, summary_inf_train_o = sess.run(
                [pr_out_o, anneal, out_o, cost, train_step, global_step, reconstruction, kl_p3, kl_p1, kl_global, kl_p2,
            ...
            if global_step_o_np % 1000 == 0:
                ...
                val_predictions_o_np, val_cost_o_np, summary_inf_test_o = sess.run(
                    ...
                    feed_dict={mask_kl_pl: kl_mask_val[rind], onehot_words_pl_val: onehot_words_val[rind],
                ...
                logger.debug('prior_predictions {}'.format(
                    [[index2token[j] for j in i] for i in pr_out_predictions[0:10, 0:50]]))
