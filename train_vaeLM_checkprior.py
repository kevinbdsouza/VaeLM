...
def train(log_dir,n_epochs,network_dict,index2token,**kwargs):
    ...
    ####
    cost,reconstruction,kl_p3,kl_p1,kl_global,kl_p2,anneal,prior_mu_o = decoder.calc_cost(mask_kl=mask_kl_pl,kl=True,sentence_word_lens=sent_word_len_list_pl,shift=shift,total_steps=total_steps,global_step=global_step,global_latent_sample=global_latent_o,global_logsig=global_logsig_o,global_mu=global_mu_o,predictions=out_o,true_input=onehot_words_pl,posterior_logsig=logsig_state_out_p,posterior_mu=mean_state_out_p,post_samples=word_state_out_p,reuse=None)

####checking out the prior
    pr_out_o, pr_global_latent_o,pr_global_logsig_o,pr_global_mu_o = decoder.run_decoder(reuse=True,units_lstm_decoder=decoder_dim,lat_words=prior_mu_o,units_dense_global=decoder_dim,sequence_length=tf.cast(sent_char_len_list_pl,dtype=tf.int32))
    ...
    for epoch in range(n_epochs):
        ...
        for count,batch in enumerate(inds):
            pr_out_predictions,anneal_c_o,train_predictions_o_np, train_cost_o_np, _, global_step_o_np,train_rec_cost_o_np,_,_,_,_,summary_inf_train_o=sess.run([pr_out_o,anneal,out_o,cost,train_step,global_step,reconstruction,kl_p3,kl_p1,kl_global,kl_p2,summary_inf_train],feed_dict={mask_kl_pl:kl_mask[batch],onehot_words_pl:onehot_words[batch],word_pos_pl:word_pos[batch],perm_mat_pl:perm_mat[batch],sent_word_len_list_pl:lat_sent_len_list[batch],sent_char_len_list_pl:sentence_lens_nchars[batch]})
	    #logger.debug('anneal const {}'.format(anneal_c))
            ...
            if global_step_o_np % 1000==0:
                ...
                logger.debug('prior_predictions {}'.format([[index2token[j] for j in i] for i in pr_out_predictions[0:10,0:50]]))
